...
A data frame with `name`, `default`, `value_str`, `tip`, `kind`, and
`shiny`.

## Details

The `kind` column is `"scalar"`, `"vector"`, `"data"`, or `"function"`.
`shiny` is `TRUE` for scalar and short-vector parameters that the
browser can edit as text. Data frames, matrices, long vectors, and
functions stay redesignable in R (`make_design(..., data = ...)`,
